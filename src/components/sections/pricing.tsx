    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-stretch">
          {pricingPlans.map((plan, index) => (
            ...
            >
              ...
              <div className="mb-6">
                <h3
                  className={`text-xl font-bold font-poppins mb-1 ${plan.highlighted ? "text-background" : "text-foreground"}`}
                >
                  {plan.name}
                </h3>
                <p
                  className={`text-sm ${plan.highlighted ? "text-card/80" : "text-muted-foreground"}`}
                >
                  {plan.description}
                </p>
              </div>
              ...
              <div className="mb-6">
                <span
                  className={`text-3xl sm:text-4xl font-bold font-poppins ${plan.highlighted ? "text-background" : "text-primary"}`}
                >
                  {plan.price}
                </span>
                <p
                  className={`text-xs mt-1 ${plan.highlighted ? "text-card/70" : "text-muted-foreground"}`}
                >
                  {plan.priceNote}
                </p>
              </div>
              ...
              <div className="space-y-3 flex-1">
                {plan.features.map((feature, fi) => (
                  <div key={fi} className="flex items-start gap-3">
                    ...
                    <span
                      className={`text-sm ${plan.highlighted ? "text-card/90" : "text-foreground/80"}`}
                    >
                      {feature}
                    </span>
                  </div>
        ...
        >
          ...
          <a
            href={WHATSAPP_LINK(
              "Halo Repasta! Saya ingin minta estimasi harga.",
            )}
            target="_blank"
            rel="noopener noreferrer"
